...
vi.mock('@tauri-apps/api/core', () => ({
  invoke: vi.fn((command: string) => Promise.resolve(command === 'hiveai_database_status' ? {
    initialized: true, engine: 'SQLite', schemaVersion: 3, migrationCount: 3, databasePath: 'hiveai.db', foreignKeysEnabled: true, lastMigrationStatus: 'ALREADY_CURRENT',
  } : command === 'hiveai_projects_list' ? [{ id: 'registered-1', name: 'Registered Fixture', originalPath: 'C:\\Projects\\Registered Fixture', normalizedPath: 'c:\\projects\\registered fixture', status: 'ACTIVE', priority: 1, preferredBuilder: 'Codex', preferredAuditor: 'GPT Audit', taskSourcePolicy: 'DISCOVER_STANDARD_FILES', registeredAt: '1', lastValidatedAt: '2', repository: { id: 'repo-1', isGitRepository: true, repositoryRoot: 'C:\\Projects\\Registered Fixture', currentBranch: 'main', headSha: 'abc', preferredRemoteUrl: 'https://github.com/example/registered.git', defaultBranch: 'main', githubOwner: 'example', githubRepo: 'registered', remotes: [{ name: 'origin', url: 'https://github.com/example/registered.git' }] } }] : {
...
describe('H!veAI M02 UI shell', () => {
  ...
  it('shows truthful native database readiness', async () => { Object.defineProperty(window, '__TAURI_INTERNALS__', { configurable: true, value: {} }); renderApp(); await waitFor(() => expect(screen.getByText('SQLite schema v3')).toBeInTheDocument()); expect(screen.getByText('3 migrations · hiveai.db · foreign keys enabled')).toBeInTheDocument(); delete (window as Window & { __TAURI_INTERNALS__?: unknown }).__TAURI_INTERNALS__; });
  it('renders persisted project registry records', async () => { Object.defineProperty(window, '__TAURI_INTERNALS__', { configurable: true, value: {} }); renderApp('/projects'); await waitFor(() => expect(screen.getByRole('heading', { name: 'Registered Fixture' })).toBeInTheDocument()); expect(screen.getByText('Git repository')).toBeInTheDocument(); expect(screen.getByText('example/registered')).toBeInTheDocument(); delete (window as Window & { __TAURI_INTERNALS__?: unknown }).__TAURI_INTERNALS__; });
});
