...
import { EmptyState, LoadingState, MetricCard, SectionHeader } from "./components/ui";
import { getCommandCenterSnapshot, listenForCommandCenterRefresh, previewSnapshot, registryFallback, type CommandCenterSnapshot } from "./commandCenter";
...
function Activity({ snapshot }: { snapshot: CommandCenterSnapshot }) {
  ...
  const items = snapshot.recentActivity.filter((item) => {
    ...
    return (!search || text.includes(search.toLowerCase())) && (kind === "ALL" || item.kind === kind);
  });
  return <section className="panel command-activity-filter"><SectionHeader title="Recent Activity" detail={`${items.length} bounded events`} /><div className="activity-filter-controls"><Search size={14} /><input aria-label="Search recent activity" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search activity" /><select aria-label="Filter activity type" value={kind} onChange={(event) => setKind(event.target.value)}><option value="ALL">All types</option>{kinds.map((value) => <option value={value} key={value}>{value}</option>)}</select></div><div className="activity-list">{items.slice(0, 50).map((item) => <div className="activity-row" key={item.id}><time>{item.occurredAt}</time><div className="activity-copy"><strong>{item.event}</strong><span>{item.projectName} · {item.kind}{item.actor ? ` · ${item.actor}` : ""}</span></div><span className="status-badge">{item.state ?? "EVIDENCE"}</span></div>)}{!items.length ? <div className="rail-empty">No matching activity evidence.</div> : null}</div></section>;
}
...
export function CommandCenterLive() {
  ...
  const refresh = React.useCallback(() => {
    ...
    if (!desktop) { setSnapshot(previewSnapshot()); setLoading(false); return; }
    setLoading(true);
    void getCommandCenterSnapshot().then((next) => { if (currentGeneration === generation.current) { setSnapshot(next && next.projects ? next : registryFallback(records)); setError(null); } }).catch((caught) => { if (currentGeneration === generation.current) { setSnapshot(registryFallback(records)); setError(caught instanceof Error ? caught.message : String(caught)); } }).finally(() => { if (currentGeneration === generation.current) setLoading(false); });
  }, [desktop, records]);
  ...
  const currentName = current?.name ?? (!desktop ? "FormuLab" : null);
  React.useEffect(() => { if (data.projects.length && (!selectedProjectId || !data.projects.some((project) => project.projectId === selectedProjectId))) selectProject(data.projects[0].projectId, true); }, [data.projects, selectedProjectId, selectProject]);
  return <div className="command-center" aria-label="Command Center overview">
    ...
    {previewNotice ? <div className="safe-notice" role="status">{previewNotice}</div> : null}
    ...
    <section className="command-kpis" aria-label="Portfolio metrics"><MetricCard label="Projects" value={String(data.kpis.projects)} detail="Registered portfolio" /><MetricCard label="Active tasks" value={count(data.kpis.activeTasks)} detail="Authoritative only" tone="blue" /><MetricCard label="Needs attention" value={String(data.kpis.needsAttention)} detail="Workflow and registry" tone="warning" /><MetricCard label="Running" value={String(data.kpis.running)} detail="Real M10 states" tone="running" /><MetricCard label="Completed tasks" value={count(data.kpis.completedTasks)} detail="Authoritative only" tone="audit" /><MetricCard label="Portfolio health" value={data.kpis.healthDetail} detail={data.kpis.authorityDetail} tone="external" /></section>
